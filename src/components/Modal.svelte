        ...
        <div class="w-full">
          <div class="m-8 my-20 max-w-[400px] mx-auto">
            ...
            <div class="space-y-4">
                ...
                <button onclick={dados.opt1.botao} class="p-3 bg-content-primary hover:brightness-90 rounded-full w-full font-semibold">{dados.opt1.nome}</button>
                {/if}
              <button class="p-3 bg-content-primary hover:brightness-90 border rounded-full w-full font-semibold" onclick={toggleModal}>{dados.opt2.nome}</button>
            </div>
          </div>
        </div>
      </div>
