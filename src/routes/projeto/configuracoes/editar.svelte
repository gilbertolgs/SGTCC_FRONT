<script lang="ts">
    ...
    let { projeto = $bindable(), cursos = [] }: Props = $props();

    let usuarioLogado: User | null = $state();
    let dadosModal: any = $state();
    let editando = $state(false);

    let txtProjeto = $state({
        'nome' : projeto.nome,
    ...
    <div class="w-full mt-2 bg-base-100 shadow-xl p-5 gap-4 text-sm rounded-xl grid grid-flow-col">
        <div>
            ...
            <div class="grid gap-2 my-2">
                ...
                <textarea bind:value={txtProjeto.descricao} cols="30" rows="10" class="input input-bordered resize-none h-full" placeholder="{projeto.descricao}"></textarea>

                <button onclick={atualizarProjeto} class="btn btn-success">
                    <span class="material-symbols-outlined mr-2">
                        done
                    </span>
                    Salvar
                </button>
                <button onclick={editarProjeto} class="btn btn-error">
                    <span class="material-symbols-outlined mr-2">
                        close
                    </span>
                    Cancelar
        ...
        {:else}
        <button onclick={editarProjeto} class="btn btn-primary flex text-center justify-center">
            <span class="material-symbols-outlined mr-2">
                edit
            </span>
            Editar
