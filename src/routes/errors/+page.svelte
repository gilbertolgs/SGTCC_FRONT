<script lang="ts">
    ...
    let connectionErrorValue: number = $state(0);

    let corpoPagina: null | {
        titulo: String,
        mensagem: String,
    } = $state(null)
...
<div class="flex flex-col items-start text-text-primary m-3">
    ...
    <button onclick={recarregarPagina} class="mt-2 p-2 rounded-md text-white font-bold cursor-pointer bg-content-primary hover:brightness-90">Recarregar</button>
</div>